...
import type { SessionPhoto } from "@/lib/supabase/types";
...
export function PhotoGallery({ sessionId, photos }: Props) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (photos.length + files.length > 5) {
      toast.error("Maximum 5 photos per session");
      return;
    }
  ...
  const deletePhoto = async (photoId: string, path: string) => {
    ...
    await supabase.from("session_photos").delete().eq("id", photoId);
